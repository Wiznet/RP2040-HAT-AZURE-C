...
static const char *g_securityTypeEnvironmentVariable = pico_az_securityTypeEnvironmentVariable;
...
static const char *g_connectionStringEnvironmentVariable = pico_az_connectionStringEnvironmentVariable;
...
static char *CopyTwinPayloadToString(const unsigned char *payload, size_t size)
{
    ...
    {
        printf("Unable to allocate %lu size buffer\n", (unsigned long)(size + 1));
    }
    else
    {
...
static bool BuildMaxMinCommandResponse(unsigned char **response, size_t *responseSize)
{
    ...
    {
        printf("Unable to allocate %lu bytes\n", (unsigned long)(responseBuilderSize + 1));
        result = false;
    }
...
static void SetEmptyCommandResponse(unsigned char **response, size_t *responseSize, int *result)
{
    ...
    {
        printf("Unable to allocate empty JSON response\n");
...
static int Thermostat_DeviceMethodCallback(const char *methodName, const unsigned char *payload, size_t size, unsigned char **response, size_t *responseSize, void *userContextCallback)
{
    ...
    *responseSize = 0;
    ...
    {
        printf("Method name %s is not supported on this component\n", methodName);
    ...
    {
        printf("Unable to parse twin JSON\n");
    ...
    {
        printf("Cannot retrieve since value\n");
        result = g_statusBadFormat;
    }
    else if (BuildMaxMinCommandResponse(response, responseSize) == false)
    {
        printf("Unable to build response\n");
        result = g_statusInternalError;
    }
    else
    {
        printf("Returning success from command request\n");
...
static JSON_Object *GetDesiredJson(DEVICE_TWIN_UPDATE_STATE updateState, JSON_Value *rootValue)
{
    ...
    {
        printf("Unable to get root object of JSON\n");
        desiredObject = NULL;
    }
    else
    {
...
static void SendTargetTemperatureReport(IOTHUB_DEVICE_CLIENT_LL_HANDLE deviceClientLL, double desiredTemp, int responseStatus, int version, const char *description)
{
    ...
    {
        printf("Unable to send reported state for target temperature.  Error=%d\n", iothubClientResult);
    }
    else
    {
...
static void SendMaxTemperatureSinceReboot(IOTHUB_DEVICE_CLIENT_LL_HANDLE deviceClientLL)
{
    ...
    {
        printf("Unable to send reported state for maximum temperature.  Error=%d\n", iothubClientResult);
    }
    else
    {
        printf("Sending maxTempSinceReboot property\n");
...
static void Thermostat_DeviceTwinCallback(DEVICE_TWIN_UPDATE_STATE updateState, const unsigned char *payload, size_t size, void *userContextCallback)
{
    ...
    printf("DeviceTwin callback invoked\n");
    ...
    {
        printf("Cannot retrieve desired JSON object\n");
    }
    else if ((targetTemperatureValue = json_object_get_value(desiredObject, g_JSONTargetTemperature)) == NULL)
    {
        printf("JSON property %s not specified.  This is NOT an error as the server doesn't need to set this, but there is no further action to take.\n", g_JSONTargetTemperature);
    }
    else if ((versionValue = json_object_get_value(desiredObject, g_IoTHubTwinDesiredVersion)) == NULL)
    {
        ...
        printf("Cannot retrieve field %s for twin.  The underlying IoTHub device twin protocol (NOT the service solution directly) should have specified this.\n", g_IoTHubTwinDesiredVersion);
    }
    else if (json_value_get_type(versionValue) != JSONNumber)
    {
        ...
        // something is fundamentally wrong with how we've received the twin and we should not proceed.
        printf("JSON field %s is not a number but must be\n", g_IoTHubTwinDesiredVersion);
    }
    else if (json_value_get_type(targetTemperatureValue) != JSONNumber)
    {
        printf("JSON field %s is not a number\n", g_JSONTargetTemperature);
    }
    else
    {
        ...
        printf("Received targetTemperature = %f\n", targetTemperature);
...
void Thermostat_SendCurrentTemperature(IOTHUB_DEVICE_CLIENT_LL_HANDLE deviceClientLL)
{
    ...
    {
        printf("Unable to send telemetry message, error=%d\n", iothubResult);
    }
...
static bool GetConnectionSettingsFromEnvironment()
{
    ...
    else
    {
        ...
        {
            printf("Environment variable %s must be either %s or %s\n", g_securityTypeEnvironmentVariable, g_securityTypeConnectionStringValue, g_securityTypeDpsValue);
            result = false;
        }
    }
...
static IOTHUB_DEVICE_CLIENT_LL_HANDLE CreateAndConfigureDeviceClientHandleForPnP(void)
{
    ...
    {
        printf("Failure to initialize client.  Error=%d\n", iothubInitResult);
        result = false;
    }
    // Create the deviceHandle itself.
    else if ((deviceHandle = CreateDeviceClientLLHandle()) == NULL)
    {
        printf("Failure creating IotHub client.  Hint: Check your connection string or DPS configuration\n");
        result = false;
    }
    ...
    {
        printf("Unable to set logging option, error=%d\n", iothubResult);
        result = false;
    }
    ...
    {
        printf("Unable to set the ModelID, error=%d\n", iothubResult);
        result = false;
    }
    ...
    {
        printf("Unable to set device method callback, error=%d\n", iothubResult);
        result = false;
    }
    ...
    {
        printf("Unable to set device twin callback, error=%d\n", iothubResult);
        result = false;
    }
    ...
    {
        printf("Unable to set auto Url encode option, error=%d\n", iothubResult);
        result = false;
    }
#ifdef SET_TRUSTED_CERT_IN_SAMPLES
    ...
    {
        printf("Unable to set the trusted cert, error=%d\n", iothubResult);
        result = false;
    }
#endif // SET_TRUSTED_CERT_IN_SAMPLES
    else
...
void pnp_simple_thermostat(void)
{
    ...
    {
        printf("Cannot read required environment variable(s)\n");
    }
    else if (BuildUtcTimeFromCurrentTime(g_ProgramStartTime, sizeof(g_ProgramStartTime)) == false)
    {
        printf("Unable to output the program start time\n");
    }
    else if ((deviceClientLL = CreateAndConfigureDeviceClientHandleForPnP()) == NULL)
    {
        printf("Failed creating IotHub device client\n");
    }
    else
    {
        printf("Successfully created device client handle.  Hit Control-C to exit program\n");
